...
function InterviewPage() {
  ...
  const handleSubmit = async () => {
    ...
    // }
      ...
      // 🔎 This helps you see exactly why it failed in the F12 Console
      console.error("Submission Error Details:", err.response?.data);
      
      // Show the specific error from Python if available
      const serverMessage = err.response?.data?.detail || "Failed to submit feedback";
      alert(`❌ ${serverMessage}`);
    }
  };

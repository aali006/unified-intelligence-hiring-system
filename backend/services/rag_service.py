...
def extract_candidate_name(query):
    """
    Try to detect a candidate name from the query.
    """
    candidates = candidates_collection.find({}, {"name": 1})
...
def get_hr_chat_response(user_query: str, stream: bool = False):
    ...
    try:
        ...
        # ------------------------------------------------
        # STEP 1: Try direct candidate lookup
        # ------------------------------------------------

        name = extract_candidate_name(user_query)
...
RESUME SUMMARY:
{candidate.get('resume_text')[:1200]}
"""
                    )
        ...
        if context_blocks:
            ...
            final_context = "No candidate information found."
        ...
        prompt = f"""
You are an HR assistant helping recruiters analyse candidates.

Rules:
- Answer ONLY using the database context.
- Do NOT mix information between candidates.
- If the information is not present, say it is not mentioned.
